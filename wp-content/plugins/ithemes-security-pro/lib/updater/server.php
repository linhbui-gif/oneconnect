...
class Ithemes_Updater_Server {
	...
	public static function request( $action, $query = array(), $data = array() ) {
		...
		if ( ! empty( $body['error'] ) ) {
			...
			return new WP_Error( $body['error']['type'], sprintf( __( 'An error occurred when communicating with the iThemes update server: %1$s (%2$s)', 'it-l10n-ithemes-security-pro' ), $body['error']['message'], $body['error']['code'] ) );
		}
