	...
	public static function get_error_explanation( $error, $package = '' ) {
		...
		switch ( $code ) {
			...
			case 'ITXAPI_License_Key_Generate_Failed':
				/* translators: 1: Product name. */
				$message = sprintf( __( '%1$s could not be licensed due to an internal error. Please try to license %2$s again at a later time. If this problem continues, please contact iThemes support.', 'it-l10n-ithemes-security-pro' ), $package_name );
				break;
		}
