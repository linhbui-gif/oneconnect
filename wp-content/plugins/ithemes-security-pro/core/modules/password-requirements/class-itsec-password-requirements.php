...
class ITSEC_Password_Requirements {
	...
	private function handle_profile_update_role( $errors, $user ) {
		...
		foreach ( ITSEC_Lib_Password_Requirements::get_registered() as $code => $requirement ) {
			...
			$message = $validated ?: esc_html__( "The provided password does not meet this site's requirements.", 'it-l10n-ithemes-security-pro' );
		...
		<div class="wp-pwd">
			...
			<div class="pw-weak">
				<label>
					...
					<?php _e( 'Confirm use of weak password', 'it-l10n-ithemes-security-pro' ); ?>
				</label>
			</div>
		</div>

		<p class="user-pass2-wrap">
			<label for="pass2"><?php _e( 'Confirm new password', 'it-l10n-ithemes-security-pro' ) ?></label><br/>
