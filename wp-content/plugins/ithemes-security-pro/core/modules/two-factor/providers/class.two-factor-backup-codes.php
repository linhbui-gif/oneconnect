...
class Two_Factor_Backup_Codes extends Two_Factor_Provider implements ITSEC_Two_Factor_Provider_On_Boardable, ITSEC_Two_Factor_Provider_CLI_Configurable {
	...
	public function user_options( $user ) {
		...
		<p id="two-factor-backup-codes">
			...
			<span class="two-factor-backup-codes-count"><?php echo esc_html( sprintf( _n( '%s unused code remaining.', '%s unused codes remaining.', $count, 'it-l10n-ithemes-security-pro' ), $count ) ); ?></span>
		</p>
	...
	public function ajax_generate_json() {
		...
		$count = self::codes_remaining_for_user( $user );
		$i18n  = esc_html( sprintf( _n( '%s unused code remaining.', '%s unused codes remaining.', $count, 'it-l10n-ithemes-security-pro' ), $count ) );
