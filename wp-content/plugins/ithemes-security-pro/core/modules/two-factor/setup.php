		...
		if ( $old < 4117 ) {
			...
			switch ( $settings['protect_user_type'] ?? null ) {
				...
				case 'custom':
					...
					$protect_user_group = [
						$upgrader->find_or_create( __( 'Two-Factor Enforced', 'it-l10n-ithemes-security-pro' ), static function ( User_Group $user_group ) use ( $settings ) {
			...
			switch ( $settings['exclude_type'] ?? null ) {
				case 'custom':
					...
					$exclude_group = [
						$upgrader->find_or_create( __( 'Disable Forced Two-Factor Authentication', 'it-l10n-ithemes-security-pro' ), static function ( User_Group $user_group ) use ( $settings ) {
			...
			switch ( $settings['allow_remember'] ?? null ) {
				...
				case 'custom':
					...
					$remember_group = [
						$upgrader->find_or_create( __( 'Allow Remembering Two-Factor', 'it-l10n-ithemes-security-pro' ), static function ( User_Group $user_group ) use ( $settings ) {
