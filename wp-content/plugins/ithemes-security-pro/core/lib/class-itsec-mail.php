...
final class ITSEC_Mail {
	...
	public function add_footer( $add_divider = true ) {
		...
		if ( ! ITSEC_Core::is_pro() ) {
			...
			$replacements = array(
				...
				'why_pro'    => sprintf( wp_kses( __( 'Why go Pro? <a href="%s">Check out the Free/Pro comparison chart.</a>', 'it-l10n-ithemes-security-pro' ), array( 'a' => array( 'href' => array() ) ) ), $security_link ),
			);
		...
		$replacements = array(
			...
			'articles_content'       => sprintf(
				wp_kses( __( 'Be the first to get the latest WordPress security news, tips, and updates on the <a href="%1$s">iThemes Blog</a>, including the <a href="%2$s">Weekly WordPress Vulnerability Report</a>.', 'it-l10n-ithemes-security-pro' ), 'mail' ),
				$articles_link,
				$vuln_report_link
			),
			...
			'tutorials_content'      => sprintf(
				wp_kses( __( 'Make sure you’re getting the most out of iThemes Security features to protect your site with our <a href="%s">free iThemes Security tutorials</a>.', 'it-l10n-ithemes-security-pro' ), 'mail' ),
				$tutorials_link
			),
			...
			'documentation_content'  => sprintf(
				wp_kses( __( 'Read iThemes Security documentation and Frequently Asked Questions on the <a href="%s">iThemes Help Center</a>.', 'it-l10n-ithemes-security-pro' ), 'mail' ),
			...
			'support_content'        => sprintf(
				wp_kses( __( 'Pro customers have the best support team available as their security team. Contact the <a href="%s">iThemes Help Desk</a> for help when you need answers.', 'it-l10n-ithemes-security-pro' ), 'mail' ),
				$support_link
			),
			'security_settings_link' => $settings_link,
			'unsubscribe_link_text'  => esc_html__( 'This email was generated by the iThemes Security plugin.', 'it-l10n-ithemes-security-pro' ) . '<br>' . sprintf( esc_html__( 'To unsubscribe from these updates, visit the %1$sSettings page%2$s in the iThemes Security plugin menu.', 'it-l10n-ithemes-security-pro' ), "<a href=\"{$settings_link}\" style=\"color: #0084CB\">", '</a>' ),
			'security_guide'         => esc_html__( 'Free WordPress Security Guide', 'it-l10n-ithemes-security-pro' ),
			'security_guide_content' => sprintf(
				wp_kses( __( 'Learn simple WordPress security tips — including 3 kinds of security your site needs and 4 best security practices for keeping your WordPress site safe with our <a href="%s">free guide</a>.', 'it-l10n-ithemes-security-pro' ), 'mail' ),
				$sec_guide_link
			),
		);
	...
	public function get_site_scanner_pro_callout() {
		...
		$template = $this->replace_all( $template, array(
			...
			'content'    =>
				wp_kses(
					__( 'iThemes Security Pro will <b>automatically update vulnerable plugins and themes for you</b> if a patch is available <b>so you don’t have to manually log in to update</b>.', 'it-l10n-ithemes-security-pro' ),
					'mail'
				) . ' ' .
				esc_html__( 'Get your site patched by iThemes Security Pro before hackers discover vulnerabilities on your site, all without doing a thing.', 'it-l10n-ithemes-security-pro' ) . '<br><br>' .
				sprintf(
					'<b>%s</b>',
					sprintf(
